...
public class Shader_Manager {
	...
	public int init(String name,GL2 gl) throws IOException {
		...
		FileInputStream fStreamV = new FileInputStream(new File(name + ".vert"));
		
//		URL u = MicroSimulation.class.getResource(name + ".vert");
		
		String vsrc = readFromStream(fStreamV);
		...
		FileInputStream fStreamF = new FileInputStream(new File(name + ".frag"));
		
		
//		u = MicroSimulation.class.getResource(name + ".frag");
		
		String fsrc = readFromStream(fStreamF);
